...
<%
	...
	System.out.println(">> 끝번호(begin) : " + p.getEnd());
	...
	// 끝페이지(endPage)가 전체페이 수(totalPage)보다 크면
			...
			<c:otherwise>
				<c:forEach var="vo" items="${list }">
					<tr>
						<td>${vo.b_idx }</td>
						<td class="align-left">
							<a href="view.jsp?b_idx=${vo.b_idx }">${vo.subject }</a></td>
						<td>${vo.writer }</td>
		...
		<tfoot>
			<tr>
				<td colspan="4">
					...
					<%--블록내에 표시할 페이지 태그 작성(시작페이지에서~끝페이지) --%>
